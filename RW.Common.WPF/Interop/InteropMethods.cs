...
public class InteropMethods {
	...
	public static IntPtr GetDC(HandleRef hWnd) {
		...
		return Handle.HandleCollector.Add(hDc, CommonHandles.HDC);
	}
	...
	public static int ReleaseDC(HandleRef hWnd, HandleRef hDC) {
		Handle.HandleCollector.Remove((IntPtr)hDC, CommonHandles.HDC);
	...
	public static string GetModuleFileName(HandleRef hModule) {
		StringBuilder buffer = new(InteropValues.Win32Constant.MAX_PATH);
		while (true) {
	...
	public static System.Windows.Point GetCursorPos() {
		...
		if (GetCursorPos(out InteropValues.POINT point)) {
			result.X = point.X;
			result.Y = point.Y;
		}
		return result;
	...
	public class Gdip {
		...
		private static void Initialize() {
			...
			int status = GdiplusStartup(out InitToken, ref input, out _);
